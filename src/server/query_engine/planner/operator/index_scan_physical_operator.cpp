...
RC IndexScanPhysicalOperator::next()
{
  ...
  while (index_scanner_->next_entry(&rid, isdelete_) == RC::SUCCESS)
  {
    Record record;
    RC rc = record_handler_->get_record(record_page_handler_, &rid, readonly_, &record);
    if (rc != RC::SUCCESS)
    {
      LOG_ERROR("Failed to get record");
      return rc;
    }

    tuple_._set_record(&record);
    bool result = false;
    rc = filter(tuple_, result);
    if (rc != RC::SUCCESS)
    {
      LOG_ERROR("Failed to filter");
      return rc;
    }
